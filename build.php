...
$repository = new Repository('tmp/lightning');
$repository->run('fetch');

$metapackage_repository = new Repository('tmp/metapackage');
$metapackage_repository->run('config', ['user.name', 'Adam Balsam']);
$metapackage_repository->run('config', ['user.email', 'balsam.adam@gmail.com']);
...
$tags = array_filter($repository->getReferences()->getTags(), function (Tag $tag) {
  return preg_match('/^[0-9]\.[0-9]+\.[0-9]+$/', $tag->getName());
});
